...
- 离线安装基础组件：containerd、runc、nerdctl、kubeadm/kubelet/kubectl。
- 在 master 节点初始化集群，并自动生成 worker 的 join command。
- 组件部署：kube-ovn、multus-cni、local-path-storage（可选）。
- 私有镜像仓库支持：同步所需镜像到私有 registry，并在部署时重写镜像地址。
...
| `nodes` | 是 | - | 节点列表，至少包含一个 `is_master: true` 的节点。 |
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `endpoint` | 否 | 私有镜像仓库域名或 IP（无协议）。 |
| `port` | 否 | 私有镜像仓库端口。 |
| `ip` | 否 | 私有镜像仓库的 IP，用于写入 `/etc/hosts`。 |
